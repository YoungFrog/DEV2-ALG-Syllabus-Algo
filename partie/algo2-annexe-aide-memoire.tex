...
%\cadre{
	...
	%\item
		%\pseudocode{long(maChaine)}
				%donne la taille (le nombre de caractères) de la chaine \pseudocode{maChaine}
	%\item
	%\pseudocode{car(maChaine,pos)}
				%donne le caractère en position \pseudocode{pos} 
				%(à partir de 1) 
				%\\dans la chaine \pseudocode{maChaine}
	%\item
	%\pseudocode{concat(maChaine1,maChaine2)}
				%concatène les chaines \pseudocode{maChaine1} 
				%et \pseudocode{maChaine2}
	%\item
	%\pseudocode{chaine(car)} 
				%transforme le caractère \pseudocode{car} 
				%en une chaine de taille 1
	%\item
	%\pseudocode{estLettre(car)} 
				%est vrai si le caractère \pseudocode{car} est une lettre
				%\\(idem pour \pseudocode{estChiffre}, 
				%\pseudocode{estMajuscule}, 
				%\pseudocode{estMinuscule})
	%\item
	%\pseudocode{majuscule(car)} 
				%donne la majuscule de la lettre \pseudocode{car}
				%(idem pour \pseudocode{minuscule})
	%\item
	%\pseudocode{position(car)} 
				%donne la position de \pseudocode{car} dans l'alphabet 
	%\item
	%\pseudocode{lettre(num)} 
				%l'inverse de la précédente
	%\item
	%\pseudocode{estEspace(car~: caractère)}
			%retourne vrai si car est un caractère d’espacement et faux sinon.
	%\item
	%\pseudocode{estPonctuation(car~: caractère)}
