...
\begin{Exercice}{La course à la case 64}
	...
	Imaginez la classe \pseudocode{Course64} qui va permettre
...
\begin{Exercice}{Mastermind}
	...
	\begin{enumerate}[label=\alph*)]
		\item
			Écrivez une classe «\pseudocode{~Combinaison~}» pour
		...
		\item
			Écrivez ensuite une classe «\pseudocode{~MasterMind~}» qui
...
\begin{Exercice}{Joyeux anniversaire}
	Un fichier \pseudocode{birthday} contient les données d’une série de personnes dans
	un ordre non déterminé. Chaque enregistrement du fichier (de type
	structuré \pseudocode{Personne}) contient les champs :
	...
	de la classe \pseudocode{Date}, sans devoir détailler son code :
...
\begin{Exercice}{Cartographie}
	...
	Un fichier \pseudocode{géo} contient les villes d’un pays avec
...
%Une commune gère la liste de ses couples mariés dans un fichier \pseudocode{unions}.
%Ce fichier est classé dans l’ordre chronologique des mariages 
%et chacun de ses enregistrements, de type \pseudocode{Union},
...
%ont été stockées dans le fichier \pseudocode{divorces}
%et ses enregistrements sont similaires à ceux de \pseudocode{unions} :
...
%que tous les enregistrements de \pseudocode{divorces}
%ont été répertoriés dans \pseudocode{unions}.

%Sur base de ces données, écrire \textbf{un} algorithme qui :

%\begin{enumerate}
%\item
	%crée un fichier \pseudocode{unionsUpdate} 
	%résultant de la mise à jour du fichier \pseudocode{unions}. 
	...
	%soient inversés dans \pseudocode{divorces}
	%par rapport à l’ordre de \pseudocode{unions}.
	%Veillez aussi à comparer les deux noms 
	%avant de supprimer un enregistrement de \pseudocode{unions}
...
\begin{Exercice}{Le jeu de la vie}
	...
	\item 
		...
		d’un fichier \pseudocode{init} de \pseudocode{Couples}
		...
		Par exemple, les couples du fichier \pseudocode{init} pour le vaisseau spatial
...
par une dans le fichier \pseudocode{lisezMoi} (en n’oubliant pas de séparer les
