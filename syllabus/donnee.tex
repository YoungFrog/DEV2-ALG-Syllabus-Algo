...
%------------------------------------
	...
	\item 
		...
		\end{itemize}
	\item
		Cette proposition introduit une classe \lda{Joueur} et le nombre de tours.
		\begin{itemize}
		\item
			\lda{circuit}: un tableau de 1 à 50 éléments de \lda{Joueur} :
			\begin{algo}
				\Class{Joueur}{
					\Private{
						\Decl{nom}{string}
					...
					\Public{\Comment{Constructeurs et accesseurs}}
				}
			\end{algo}
	...
	\item
		...
		\begin{itemize}
		\item
			\lda{circuit}: un tableau de 2 éléments \lda{Joueur}, une classe différente.  
			\begin{algo}
				\Class{Joueur}{
					\Private{
...
%---------------------------------------
	...
	(un booléen à \lda{vrai} si cette somme est assurée, \lda{faux} sinon).
...
%---------------------------------
	...
	\begin{algo}
		\MethodSign{effectuerRéservation}{demande : DemandeRéservation}{integer}
	\end{algo}	
	\begin{itemize}
		\item 
			Elle retourne le numéro de la chambre qui a été choisie
			ou $-1$ si la réservation n'a pas pu se faire.
		\item 
		...
		\item 
			Vous pouvez supposer qu'il existe une méthode donnant le nombre de jours
			séparant deux dates données.
	\end{itemize}
