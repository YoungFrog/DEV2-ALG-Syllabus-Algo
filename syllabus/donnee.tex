...
%====================================
	...
	est lié étroitement au choix de la représentation.
...
%=======================================
	...
	nous avons commencé le cours d'algorithmique de DEV$_{1}$
...
%==================================
	...
	dans le cours d'algorithmique de DEV$_{3}$ : 
...
\begin{Exercice}{Un jeu de poursuite}
	...
	\begin{itemize}
		...
		\item 
			...
			\begin{itemize}
				\item 
					la méthode «\lda{~jouerCoup~}» qui joue pour un joueur et
					indique s'il a rattrapé l’autre joueur (sans
					répétition si on arrive sur une case \lda{vrai})
				\item 
					la méthode «\lda{~jouerTour~}» effectue la même tâche
					mais avec répétition si on arrive sur une case
					\lda{vrai}. On fera évidemment appel à la méthode
					ci-dessus.
				\item 
					la méthode «\lda{~joueurSuivant~}» qui permet de passer au
					joueur suivant.
			\end{itemize}
	\end{itemize}
	
	Avec ces 3 méthodes, la méthode publique
	«\lda{~jouer~}» devient triviale.
\end{Exercice}
...
\begin{Exercice}{La course à la case 64}
	...
	tableau \lda{joueurs} de quatre chaines contient les noms et prénoms des
	...
	lancement du dé est simulé par l'algorithme \lda{lancerDé(
	)} qui retourne une valeur aléatoire entre 1 et 6.

	Imaginer la classe \lda{Course64} qui va permettre
...
\begin{Exercice}{L'ensemble}
		...
		\begin{LDA}
			...
			\RComment T est le type des éléments de l'ensemble
				...
				\MethodSign{listeÉléments}{}{Liste de T}
