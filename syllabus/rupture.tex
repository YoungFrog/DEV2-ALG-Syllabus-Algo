...
%===================================
	...
	Cet algorithme est notamment adapté pour un problème comme celui-ci :
	\begin{quotation}
		\slshape
	...
	Nous verrons que l'algorithme que nous allons pouvoir écrire va dépendre
	fortement de la façon dont la liste est triée (ou pas).
	...
	\begin{itemize}
		\item 
			...
			et identifier si le tri des éléments est adapté au problème.
		\item 
...
\subsection*{Introduction}
	...
	par exemple des objets Date, où l’opérateur de comparaison est remplacé par la méthode
	«~estAntérieure()~».
	
	Plus généralement, les données composées de plusieurs
...
\subsection*{Exemple de classement triple}
	...
	trois lettres serait acceptable.
...
%==============================
	...
	\footnote{%
		...
		la taille l'ensemble lorsqu'on commence à le traiter.
	}. 
	...
	clé composée pour le classement de ces données, la succession des
...
\subsection*{Rupture de niveau 0}
	...
	\begin{LDA}
		\Algo{RuptureNiveau0}{\Par{etudiants}{liste d'Etudiant}}{}
			\Decl{etd}{Etudiant}
			\Decl{i}{entier}
			\Let i \Gets 0
...
\subsection*{Rupture de niveau 1}
	...
	\begin{LDA}
		\Algo{RuptureNiveau1}{\Par{etudiants}{liste d'Etudiant}}{}
			\LComment on suppose les données classées en majeur sur l’option
			\Decl{etd}{Etudiant}
			\Decl{saveOption}{chaine}
			\Decl{cpt}{entier}
			\Decl{etudiant}{Etudiant}
	
			\Let i \Gets 0
			\While{i < etudiants.taille()}
				\Let saveOption \Gets etudiants.get(i).option
				\Let cpt \Gets 0
				\While{i < etudiants.taille() ET saveOption = etudiants.get(i).option}
					\Let cpt \Gets cpt + 1
					\Let i \Gets i + 1
				\EndWhile
		...
		\Algo{RuptureNiveau2}{\Par{etudiants}{liste d'Etudiant}}{}
			...
			\While{i < etudiants.taille()}
				...
				\While{i < etudiants.taille() ET saveOption = etudiants.get(i).option}
					...
					\Let cpt \Gets 0
					\While{i < etudiants.taille() 
							\Stmt \Indent \Indent ET saveOption = etudiants.get(i).option 
							\Stmt \Indent \Indent ET saveAnnéeNaissance = etudiants.get(i).dateNaissance.année}
						\Let cpt \Gets cpt + 1
						\Let i \Gets i + 1		
					\EndWhile
					\Write cpt, «~étudiant dans l’option~», saveOption, 
...
\begin{Exercice}{La chasse au gaspi [rupture de niveau 1]}
	...
	Cette liste présente la structure d’enregistrement \textbf{Job} suivante 
	et est ordonné croissant \textbf{en majeur} sur le champ \textsf{login}  :
...
\begin{Exercice}{Compter les étudiants [rupture de niveau 2]}
	...
	Supposons que la structure \textsf{Etudiant} contienne également
...
\begin{Exercice}{Les fanas d'info  [rupture de niveau 2]}
	...
	dont les éléments sont de type \textbf{Participant}.
	...
	et en mineur sur le champ \textsf{nom}}, 
