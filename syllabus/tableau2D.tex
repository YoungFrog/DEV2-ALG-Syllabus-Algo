...
% ===================
	...
	En \abbr{dev$_1$}, nous avons introduit les tableaux à une dimension.
...
% ====================
	...
	%====================
		...
		entre crochets.
		Comme en \abbr{dev$_1$}, on considère que la première ligne 
	...
	%======================
		...
		Cela ne change pas le fait que la valeur de \lda{nombres[2,3]} 
		est toujours $23$. 
	...
	%======================
		...
		La version \bsc{Java} :
		\listing*[linerange=11-18,basicstyle=\footnotesize\vtt]{java}{java/src/main/java/dev2/algo/tab/Exemple1.java}
		
		% \medskip
		\begin{infotbox}{Syntaxe Java}
			On vous renvoie à votre cours de \bsc{Java}
			pour les précisions sur la syntaxe \bsc{Java}.
			\\
			Notez juste ici :
			\begin{itemize}
				\item
		...
		\begin{algo}
		\Algo{initialiser}{\Par{entiers}{\Array{n$\times$m}{integers}}, \Par{valeur}{integer}}{}{
			\Fori{i}{0}{n-1}{
				\Fori{j}{0}{m-1}{
					entiers[i,j] \Gets valeur\;
				}
			}
		}
		\end{algo}
		...
		\Algo{traiterStock1Jour}{\Par{cpt}{\Array{7$\times$10}{integers}}, \Par{jour}{integer}}{}{
		...
		En \bsc{Java} :
		\listing*[linerange=11-22,basicstyle=\footnotesize\vtt]{java}{java/src/main/java/dev2/algo/tab/Exemple2.java}
		\listing*[linerange=24-34,basicstyle=\footnotesize\vtt]{java}{java/src/main/java/dev2/algo/tab/Exemple2.java}

	\subsection{Exercices}
			
		Ces exercices ne nécessitent pas encore de parcours du tableau
		(pas de boucle). 
		Nous verrons les algorithmes de parcours juste après.
		...
		\begin{minipage}{1.0\linewidth}% pour tout garder ensemble
		\begin{Exercice}{Case existe ?}
			Écrire un algorithme qui reçoit un tableau d'entiers
			(à $n$ lignes et $m$ colonnes)
			ainsi que des coordonnées
			et qui retourne un booléen indiquant 
			si ces coordonnées désignent bien une case du tableau,
			c'est-à-dire qu'elles sont dans le bon intervalle
			($0$ à $n-1$ et $0$ à $m-1$).
			
			\begin{algo}
				\AlgoSign{existe}{\Par{tab}{\Array{n $\times$ m}{integers}}, \Par{lg, col}{integers}}{boolean}\;
			\end{algo}
		\end{Exercice}
		\begin{Solution}
			\begin{algo}
				\Algo{existe}{\Par{tab}{\Array{n $\times$ m}{integers}}, \Par{lg, col}{integers}}{boolean}{
					\Return $ 0 \le{} \mathit{lg} < n$ ET $0 \le{} \mathit{col} < m$\;
				}
			\end{algo}
		\end{Solution}
		\end{minipage}
		...
		\begin{Exercice}{Assigner une case}
			...
			\begin{algo}
				\AlgoSign{assigner}{\Par{tab}{\Array{n $\times$ m}{integers}}, \Par{lg, col, val}{integers}}{}\;
			\end{algo}
		\end{Exercice}

		\begin{Solution}
			\begin{algo}
				\Algo{assigner}{\Par{tab}{\Array{n $\times$ m}{integers}}, \Par{lg, col, val}{integers}}{}{
...
% ================================================
	...
	%====================================
		...
		\begin{algo}
			...
			\Algo{afficherLigne}{%
				...
				\ForAll{col : numéro de colonne 
					\Comment{Pour tous les numéros de colonne ($0$ à $m-1$)}}{
					\Write tab[ lg, col ] \RComment{On peut faire autre chose qu'afficher}
				}
			}
		      \end{algo}
		      N.B. : Le symbole $\forall$ se lit \og pour tout\fg{} et a exactement cette signification.
		...
		\listing*[linerange=9-13,basicstyle=\footnotesize\vtt]{java}{java/src/main/java/dev2/algo/tab/Parcours1D.java}
		...
		\listing*[linerange=15-19,basicstyle=\footnotesize\vtt]{java}{java/src/main/java/dev2/algo/tab/Parcours1D.java}
		...
		\listing*[linerange=21-25,basicstyle=\footnotesize\vtt]{java}{java/src/main/java/dev2/algo/tab/Parcours1D.java}

		\paragraph{La diagonale montante.}
		Pour la diagonale montante, 
		on peut envisager deux solutions :
		\begin{itemize}
		\item avec deux indices, ou
		\item avec un seul indice, en se basant sur le fait que
		  \begin{displaymath}
		...
		\listing*[linerange=27-34,basicstyle=\footnotesize\vtt]{java}{java/src/main/java/dev2/algo/tab/Parcours1D.java}
		\listing*[linerange=36-41,basicstyle=\footnotesize\vtt]{java}{java/src/main/java/dev2/algo/tab/Parcours1D.java}
		\listing*[linerange=43-48,basicstyle=\footnotesize\vtt]{java}{java/src/main/java/dev2/algo/tab/Parcours1D.java}

	\subsection{Parcours des deux dimensions}
	%========================================
	
		Les parcours précédents ne concernaient qu'une partie du tableau.
		Voyons à présent comment visiter toutes les cases du tableau.

		\paragraph{Parcours par lignes et par colonnes.}
		%---------------------------------------------------
			
			Les deux parcours les plus courants sont les parcours \og ligne par ligne\fg{}
			et \og colonne par colonne\fg.
			...
			\listing*[linerange=9-15,basicstyle=\footnotesize\vtt]{java}{java/src/main/java/dev2/algo/tab/Parcours2D.java}
			\listing*[linerange=17-23,basicstyle=\footnotesize\vtt]{java}{java/src/main/java/dev2/algo/tab/Parcours2D.java}
			...
			\begin{algo}
				...
				\Algo{afficherLigneParLigne}{\Par{tab}{\Array{n $\times$ m}{T}}}{}{
					On commence à la 1\iere{} case, en haut à gauche.\;
					\Repeat{$n*m$}{
						\Write tab[lg,col] \RComment{On peut faire autre chose qu'afficher}
						\uIf{on n'est pas sur le bord droit} {
			...
			Et en \bsc{Java} : 
			\listing*[linerange=25-37,basicstyle=\footnotesize\vtt]{java}{java/src/main/java/dev2/algo/tab/Parcours2D.java}
		...
		%---------------------------------------
			...
			Illustrons-le au travers d'un algorithme
			qui cherche un élément particulier.
			et retourne un booléen indiquant si
			l'élément a été trouvé ou pas.

			Avec deux boucles, la solution la \textbf{plus simple}
			est celle qui introduit un \textbf{booléen}.
				...
				\Algo{chercherElément}{\Par{tab}{\Array{n $\times$ m}{T}}, \Par{élt}{T}}{boolean}{
					...
					\While{lg $<$ n ET NON trouvé}{
						...
						\While{col $<$ m ET NON trouvé}{
							trouvé \Gets \textit{tab[lg, col] == élt}\;
							col++\;
						}
						lg++\;
					}
			...
			S'il est important de connaitre la position de l'élément trouvé,
			il faut veiller à ne pas incrémenter la ligne et/ou la colonne 
			lorsque l'élément est trouvé. 
			Ce qui donne :
			...
			\listing*[linerange=39-59,basicstyle=\small\vtt]{java}{java/src/main/java/dev2/algo/tab/Parcours2D.java}

			% \newpage
			\paragraph*{Version avec 1 boucle.}

			Utilisons la même stratégie pour le parcours avec une seule boucle.
			Avec une boucle, 
			on peut facilement utiliser la version avec le \textbf{test dans la condition}.
		...
		%----------------------------------------------------
			...
			Envisageons un parcours plus difficile 
			où on inverse le sens du parcours d'une ligne à l'autre
			...
			\begin{algo}
				...
				\Algo{afficherElémentsSerpent}{\Par{tab}{\Array{n $\times$ m}{T}}}{}{
					Se placer sur la 1\iere{} case\;
					Commencer à se déplacer à droite\;
					\Repeat{$n*m$}{
						\Write tab[lg, col]\;
						\eIf{le déplacement dans le sens courant (droite ou gauche) ne sort pas du tableau}{
							Se déplacer dans le sens courant\;
						}{
							Passer à la ligne suivante\;
							Inverser le sens de déplacement (droite $\Longleftrightarrow$ gauche)\;
						}
					}
				}
			\end{algo}

			Pour retenir le sens du déplacement, nous allons utiliser
			un entier ($-1$ pour gauche et $1$ pour droite)
			ce qui va faciliter le calcul du déplacement.
...
% ===================
	...
	\begin{Solution}
		\begin{algo}
                  \Algo{pinceauZebre}{\Par{tab}{\Array{n $\times$ n}{entiers}}}{}{
                    \Assign{colDepart}{0}\;
                    \Fori[]{lg}{0}{n-1}{
                  ...
                  \Algo{pinceauSpirale}{\Par{tab}{\Array{n $\times$ n}{entiers}}}{}{
                    \Assign{lg}{0}\;
                    \Assign{col}{0}\;
                    \Assign{dirLg}{0}\;
                    \Assign{dirCol}{1}\;
                ...
                \begin{algo}
                  \Algo{tournerADroite}{\Par{dirLg\InOut}{entier}, \Par{dirCol\InOut}{entier}}{}{
					\Comment{Pour Java, en faire un objet pour pouvoir le changer dans la méthode}
				 	\Assign{dirLg}{dirCol}\;
                    \Assign{dirCol}{-dirLg}\;
                  }
